...
import { ChevronUp, ChevronDown, ChevronsUpDown, Plus, Search, ArrowLeft, LayoutPanelTop } from 'lucide-react';
...
export default function TablesPage() {
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <Link
            href={`/restaurants/${restaurantId}/tables/${row.original.id}`}
            className="font-medium text-foreground hover:text-primary transition-colors"
          >
            <span className="font-semibold">#{row.original.tableNumber}</span>
            {row.original.name && (
              <span className="text-muted-foreground ml-1.5 text-xs">({row.original.name})</span>
            )}
          </Link>
        ),
      },
      {
        ...
        header: 'Capacity',
        cell: ({ row }) => {
          const min = row.original.minimumCapacity;
          const max = row.original.maximumCapacity;
  ...
  return (
    <PageWrapper
      title={t('Tables')}
      description={restaurant ? t('Manage tables for {name}', { name: restaurant.name }) : t('Manage tables')}
      actions={
        <div className="flex items-center gap-2">
          ...
          <Link href={`/restaurants/${restaurantId}/tables/floor-plan`}>
            <Button variant="outline" size="sm">
              <LayoutPanelTop className="h-4 w-4 mr-1.5" />
              {t('Floor Plan')}
            </Button>
          </Link>
          <Link href={`/restaurants/${restaurantId}/tables/create`}>
            <Button>
      ...
      <div className="space-y-4">
              ...
              {isLoading ? (
                  ...
                  <td colSpan={columns.length} className="h-32 text-center">
                    <div className="flex flex-col items-center justify-center text-muted-foreground">
                      ...
                      {search || statusFilter ? (
                        ...
                        <Link
                          href={`/restaurants/${restaurantId}/tables/create`}
                ...
                table.getRowModel().rows.map((row) => (
                  <tr
                    ...
                    onClick={() => router.push(`/restaurants/${restaurantId}/tables/${row.original.id}`)}
                  >
        ...
        {!isLoading && filteredTables.length > 0 && (
           <p className="text-sm text-muted-foreground">
            {t('{count} table(s)', { count: filteredTables.length })}
