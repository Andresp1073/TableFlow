...
export default function PurchaseOrderDetailPage() {
  ...
  const handleSubmit = () => {
    submitPO.mutate({ restaurantId, orderId }, {
      onSuccess: () => { toast.success('Order submitted'); router.refresh(); },
  ...
  const handleApprove = () => {
    approvePO.mutate({ restaurantId, orderId }, {
      onSuccess: () => { toast.success('Order approved'); router.refresh(); },
  ...
  const handleCancel = () => {
    cancelPO.mutate({ restaurantId, orderId, reason: 'Cancelled by user' }, {
      onSuccess: () => { toast.success('Order cancelled'); router.refresh(); },
