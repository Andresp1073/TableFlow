  ...
  return (
    <div className="flex items-start gap-3 py-2.5 border-b last:border-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-sm font-medium mt-0.5">{value ?? <span className="text-muted-foreground italic">{t('Not set')}</span>}</div>
      </div>
    </div>
  );
}
  ...
  if (isError || !data) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="text-lg font-medium mb-2">{t('Failed to load customer profile')}</p>
        <p className="text-sm text-muted-foreground mb-4">{t('The customer could not be found or an error occurred.')}</p>
        <Link href="/customers"><Button variant="outline">{t('Back to Customers')}</Button></Link>
      </div>
    );
  }
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {data.isVip && <Badge variant="warning"><Star className="h-3 w-3 mr-1" />{t('VIP')}</Badge>}
            </div>
            <p className="text-sm text-muted-foreground">{t('Customer since')} {new Date(data.createdAt).toLocaleDateString()}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle className="text-lg">{t('Personal Information')}</CardTitle>
          </CardHeader>
          <CardContent>
