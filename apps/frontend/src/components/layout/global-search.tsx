    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="top-[15%] -translate-y-0 sm:top-[20%]">
        <DialogHeader className="sr-only">
          ...
          <DialogDescription>Search across the application</DialogDescription>
        </DialogHeader>
        <SearchBar
          placeholder="Search restaurants, reservations, customers..."
          className="mt-2"
          autoFocus
        />
        <div className="px-1 py-2 text-xs text-muted-foreground">
          <p>Press <kbd className="rounded border bg-muted px-1 font-mono">↑</kbd><kbd className="rounded border bg-muted px-1 font-mono">↓</kbd> to navigate, <kbd className="rounded border bg-muted px-1 font-mono">Enter</kbd> to select</p>
        </div>
      </DialogContent>
    </Dialog>
  );
